...
- [ ] The overall concept -> tasks are functions, groups are modules, and the GUI is constructed from the modules at a given location (package)
- [ ] The `__init__.py` file of the package
- [ ] The modules in the package (is package a good name?)
- [ ] The `@exec_ui` decorator
- [ ] Icons
- [ ] Kernels
- [ ] Starting the Task GUI
...
Create a folder `yakka` that will be our Task GUI package. In the folder create an empty file `\\__init__.py` and a file named `hello.py`.
