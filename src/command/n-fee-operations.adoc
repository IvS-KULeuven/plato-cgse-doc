...
FM::
+
.CCD numbering and pixel coordinates for the FM normal camera on every CCD (CCD_PIXn reference frames). The thicker black border lines represent the readout registers.
[#fig-ccd-numbering-coordinates-fm]
image::../images/ccd-numbering-coordinates-fm.png[width=80%,align=center]

EM::
+
.CCD numbering and pixel coordinates for the EM normal camera on every CCD (CCD_PIXn reference frames). The thicker black border lines represent the readout registers.
